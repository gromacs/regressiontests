...
sub setup_vars()
{
    ...
    if (system("$gmx_cmd$suffix $mdrun_cmd -pme gpu > stdout 2>&1") != 0) {
        $mdrun_supports_pme_on_gpus = 1;
    }
    unlink("stdout");
    ...
    foreach my $dir ( @subdirs ) {
        ...
        if ($test_name =~ /nbnxn/ && 0 < find_in_file("^Using .* 8x8 non-bonded kernels", "$dir/md.log")) {
            if ($mdparams =~ /-nb/) {
                ...
                prepare_run_with_different_task_decomposition($test_name, "cpu-only", \$new_dir, \$new_input_dir, \$new_test_name);
                $$nn++;
                $$npassed += test_case $new_dir, $new_input_dir, $new_test_name;
            }
        }
        # If GPU testing is required, and PME on GPU is supported,
        # arrange to run PME tests on both CPU and GPU.
        if ($test_name =~ /(?<!lj)pme/ && $gpu_id && $mdrun_supports_pme_on_gpus)
        {
            if ($mdparams =~ /-pme/) {
                print("Because you have set -pme in the -mdparam string, the test\n" .
                      "harness will not attempt to rerun PME tests on both CPU and GPU.\n");
            } else {
                print("Re-running $test_name using GPU-based PME\n");

                my ($new_dir, $new_input_dir, $new_test_name);
                prepare_run_with_different_task_decomposition($test_name, "pme-gpu", \$new_dir, \$new_input_dir, \$new_test_name);
                $$nn++;
                $$npassed += test_case $new_dir, $new_input_dir, $new_test_name;
            }
        }

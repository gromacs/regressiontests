...
# Calculates radius from two eigenvectors and compares it to the expected radius
sub check_radius {
    my ( $File, $ind1, $ind2, $indR, $absTol ) = @_;
    ...
    while (my $Line = <FIL>) {
        ...
        if (defined($Line)) {
            ...
            my $rad      = sqrt(($ev1 - $ev1start)**2 + ($ev2 - $ev2start)**2) + $radstart;
            my $absError = abs($rad - $rad_ref);
...
sub test_essentialdynamics {
  ...
  $dir = "radfix";
  chdir $dir || die;
  $retval = run_single_ed_system($dir, "-outfrq 1 -radfix 1-2 -radstep 0.002 -eqsteps 1");
  if ( 0 == $retval ) {
      # Compare the essential dynamics output to the reference:
      $nerror += compare_xvg_column($edref, $edofn, 0, 0.0   );  # 0 = Time (ps), must match exacly!
      $nerror += compare_xvg_column($edref, $edofn, 1, 0.05  );  # 1 = RMSD (nm)
      $nerror += compare_xvg_column($edref, $edofn, 4, 0.0   );  # 4 = RADFIX radius (nm), must match exactly
      $nerror += check_radius($edofn, 2, 3, 4, 1e-5);            # RADFIX radius is calculated from columns 2-3, must match column 4
  } else {
      $nerror++;
  }
  chdir "..";
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += check_radius($edofn, 2, 3, 4, 1e-5);             # RADACC radius is calculated from columns 2-3, must match column 4
      $nerror += check_monotonicity($edofn, 4, +1);               # RADACC radius must increase monotinically
  } else {
      $nerror++;
  }
  chdir "..";

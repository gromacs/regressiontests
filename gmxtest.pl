...
sub run_single_ed_system {
  ...
  if ( $nerror == 0 ) { # If we already had errors, there is no use in going on ...
    ...
    $nerror = run_mdrun($tmpi_ranks, $omp_threads, $mpi_ranks, $npme_ranks, $gpu_id, $mdprefix, "-quiet -ei $edifn -eo $edofn", "grompp.mdp");
  }
...
sub test_essentialdynamics {
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += compare_xvg_column($edref, "$dir.xvg", 0, 0.0    );  # 0 = Time (ps), must match exacly!
      $nerror += compare_xvg_column($edref, "$dir.xvg", 1, 0.05   );  # 1 = RMSD (nm)
      $nerror += compare_xvg_column($edref, "$dir.xvg", 2, 0.01   );  # 2 = EV 1 projection (nm)
      $nerror += compare_xvg_column($edref, "$dir.xvg", 3, 250    );  # 3 = EV 1 V_fl (kJ/mol) 
      $nerror += compare_xvg_column($edref, "$dir.xvg", 4, 500    );  # 4 = EV 1 fl_forces (kJ/mol/nm) 
      $nerror += compare_xvg_column($edref, "$dir.xvg", 5, 0.01   );  # 5 = EV 2 projection (nm)
      $nerror += compare_xvg_column($edref, "$dir.xvg", 6, 250    );  # 6 = EV 2 V_fl (kJ/mol) 
      $nerror += compare_xvg_column($edref, "$dir.xvg", 7, 500    );  # 7 = EV 2 fl_forces (kJ/mol/nm) 
  } else {
      $nerror++;
  }
  chdir "..";

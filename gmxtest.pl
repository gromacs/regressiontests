...
# Reads some test-specific command line arguments for the gmx make_edi command line 
# from text file. This way, extra essential dynamics tests can be added without
# having to modify this source code.
sub readEdiArgs { 
  my $fn = shift;

  open(FILE, "$fn") || die("FAILED: Can not read make_edi command line arguments from $fn");
  my $args = <FILE>;  # read first line only, ignore the rest (there shouldn't be any!)
  close FILE;
...
sub test_essentialdynamics {
  my $edifn  = "sam.edi";
  my $edofn  = "edsam.xvg";
  my $argsfn = "argsForMakeEdi.txt";  # Text file containing test-specific command line arguments for make_edi
  my $nerror = 0;
  my $ntest  = 0;
  ...
  foreach my $dir ( glob("*") ) {
    if ( -d $dir) {
      ...
      my $ediArgs = readEdiArgs($argsfn);
#     $nerror += do_system("echo 0 | $progs{'make_edi'} -f eigenvec.trr $ediArgs -o $edifn 1>$makeEdi_out 2>$makeEdi_err");
      $nerror += do_system("echo 0 | $progs{'make_edi'} -f eigenvec.trr $ediArgs -o $edifn -quiet"); # debug ...
      ...
      if ( $nerr > 0 ) {
        ...
        $nerror += compare_xvg_column("edsam_reference.xvg", $edofn, 2, 1.0e-6);  # 2 = EV projection, last value is the tolerance in nm
      }
      chdir "..";
    }
  }

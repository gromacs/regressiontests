...
sub check_monotonicity {
    ...
    if (!$nerr) {
        ...
        print("\nERROR: values of column $iCol are NOT $buf monotonically!\n");
        return 1;
    }
}
...
sub test_essentialdynamics {
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += check_monotonicity($edofn, 2, +1);              # EV 1 projection must increase monotonically
  } else {
      $nerror++;
  }
  chdir "..";
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += check_monotonicity($edofn, 4, +1);               # RADACC radius must increase monotonically
  } else {
      $nerror++;
  }
  chdir "..";
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += check_monotonicity($edofn, 4, -1);               # RADCON radius must decrease monotonically towards zero.
  } else {
      $nerror++;
  }
  chdir "..";
  ...
  if ( 0 == $retval ) {
      ...
      $nerror += compare_xvg_column($edref, $edofn, 0, 0.0    );  # 0 = Time (ps), must match exacly!
      $nerror += compare_xvg_column($edref, $edofn, 1, 0.05   );  # 1 = RMSD (nm)
      $nerror += compare_xvg_column($edref, $edofn, 2, 0.001  );  # 2 = EV 1 projection (nm)
      $nerror += compare_xvg_column($edref, $edofn, 3, 1      );  # 3 = EV 1 V_fl (kJ/mol) 
      $nerror += compare_xvg_column($edref, $edofn, 4, 10     );  # 4 = EV 1 fl_forces (kJ/mol/nm) 
      $nerror += compare_xvg_column($edref, $edofn, 5, 0.001  );  # 5 = EV 2 projection (nm)
      $nerror += compare_xvg_column($edref, $edofn, 6, 1      );  # 6 = EV 2 V_fl (kJ/mol) 
      $nerror += compare_xvg_column($edref, $edofn, 7, 10     );  # 7 = EV 2 fl_forces (kJ/mol/nm) 
  } else {
      $nerror++;
  }
  chdir "..";

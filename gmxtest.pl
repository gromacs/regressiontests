...
#      $nerror += do_system("echo 0 | $progs{'make_edi'} -f eigenvec.trr -outfrq 1 -linfix 1 -linstep 0.0013 -o $edifn 1>$makeEdi_out 2>$makeEdi_err");
      $nerror += do_system("echo 0 | $progs{'make_edi'} -quiet -f eigenvec.trr -outfrq 1 -linfix 1 -linstep 0.0013 -o $edifn"); # debug ...
      # Check whether we get the expected .edi file
      my $nerr = compare_textfiles($edifn, "sam_reference.edi");
      if ( $nerr > 0 ) {
        $nerror += $nerr;
        print("ERROR: Produced .edi file does not match the reference!");
      }

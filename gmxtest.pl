...
sub test_essentialdynamics {
  ...
  $dir = "linfix";
  chdir $dir;
  run_single_ed_system($dir, "-outfrq 1 -linfix 1 -linstep 0.0013");
  ...
  $nerror += compare_xvg_column($edref, $edofn, 1, 0.001 );  # 1 = RMSD (nm)
  ...
  $dir = "linacc";
  chdir $dir;
  run_single_ed_system($dir, "-outfrq 2 -linacc 1 -accdir +1");

  # Compare the essential dynamics output to the reference:
  $nerror += compare_xvg_column($edref, $edofn, 0, 0.0   );  # 0 = Time (ps), must match exacly!
  $nerror += compare_xvg_column($edref, $edofn, 1, 0.001 );  # 1 = RMSD (nm)
  $nerror += compare_xvg_column($edref, $edofn, 2, 1.0e-6);  # 2 = EV projection (nm) LINACC, last value is the tolerance
  chdir "..";
  # ----------------------------------------------------------------------------
  $ntest++;
  $dir = "radfix";
  chdir $dir;
  run_single_ed_system($dir, "-outfrq 1 -radfix 1-2 -radstep 0.001");

  # Compare the essential dynamics output to the reference:
  $nerror += compare_xvg_column($edref, $edofn, 0, 0.0   );  # 0 = Time (ps), must match exacly!
  $nerror += compare_xvg_column($edref, $edofn, 1, 0.0001);  # 1 = RMSD (nm)
  $nerror += compare_xvg_column($edref, $edofn, 2, 1.0e-6);  # 2 = EV1 projection RADFIX (nm)
  $nerror += compare_xvg_column($edref, $edofn, 3, 1.0e-6);  # 3 = EV2 projection RADFIX (nm)
